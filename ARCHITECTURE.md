...
**Automated response is a suggest-and-approve loop, not literal automation, and only does what the system can actually do.** Real DSP-style responses ("disable this share," "kill this process") assume the tool operates the file server — logikos-dsp's agent is a client that watches shares, it doesn't administer them, so those aren't implemented; the only response action in v1 is a **webhook notification** (`packages/backend/src/responseActions/webhook.ts`), POSTing alert details to `RESPONSE_WEBHOOK_URL` (Slack/PagerDuty/generic). Nothing fires automatically regardless of severity: HIGH/CRITICAL alerts get a `ResponseAction` row created in `PENDING` status (by `packages/backend/src/rules/ransomwareRate.ts` for CRITICAL ransomware-rate alerts, and by the classification worker for HIGH sensitive-data alerts — see below), and it only executes once an ADMIN calls `POST /response-actions/:id/approve`; `POST /response-actions/:id/reject` dismisses a suggestion as a false positive without executing anything. This is a firm product requirement, not a placeholder to relax later. `ResponseAction` is its own table (not columns bolted onto `Alert`) so the alert lifecycle (open/acknowledged/resolved) and the response lifecycle (pending/rejected/executed/failed) stay separate concerns, leaving room for a second action type (e.g. local-path file quarantine) without reshaping `Alert` again.
...
- File quarantine and any response action that requires *writing* to a watched source — the SMB connector is intentionally read-only (see above), and even for local paths this needs its own design pass. Webhook notification is the only response action in v1 (see "Automated response" above).
